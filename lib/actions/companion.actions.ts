...
export const getAllCompanions = async ({ limit = 10, page = 1, subject, topic }: GetAllCompanions) => {
    ...
    // Get companion IDs from the current page
    const companionIds = companions.map(c => c.id);
    
    // Fetch only bookmarks for these companions
    ...
    const bookmarkedCompanionIds = new Set(bookmarks?.map(b => b.companion_id) || []);
    
    const companionsWithBookmarks = companions.map((companion)=>{
      return {
        ...companion,
        bookmark: bookmarkedCompanionIds.has(companion.id)
      }
    })
...
export const newSessionPermission = async () => {
  const { userId , has } = await auth()
  const supabase = createSupabaseClient()
  let limit = 0
